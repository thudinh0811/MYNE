...
void Assets::addSound(const std::string& soundName, const std::string& path)
{
    ...
    else
    {
        ...
        m_soundMap[soundName].setVolume(50);
    }
}

void Assets::addMusic(const std::string& soundName, const std::string& path)
{
    m_soundBufferMap[soundName] = sf::SoundBuffer();
    if (!m_soundBufferMap[soundName].loadFromFile(path))
    {
    ...
    {
        std::cout << "Loaded Sound:    " << path << std::endl;
        m_musicMap[soundName] = sf::Sound(m_soundBufferMap[soundName]);
        m_musicMap[soundName].setVolume(50);
        m_musicMap[soundName].setLoop(true);
    }
}
...
sf::Sound& Assets::getMusic(const std::string& soundName)
{
    assert(m_musicMap.find(soundName) != m_musicMap.end());
    return m_musicMap.at(soundName);
}


void Assets::addShader(const std::string& shaderName, const std::string& path)
{
    m_shaderMap[shaderName] = path;
    std::cout << "Loaded Shader:  " << path << std::endl;
}

const std::string& Assets::getShader(const std::string& shaderName)
{
    assert(m_shaderMap.find(shaderName) != m_shaderMap.end());
    return m_shaderMap.at(shaderName);
}

void Assets::setMusicVolume(int v)
{
    for (auto& [key, val] : m_musicMap)
    {
        val.setVolume(v * 10);
    }
}

void Assets::setSFXVolume(int v)
{
    for (auto& [key, val] : m_soundMap)
    {
        val.setVolume(v * 10);
    }
}
